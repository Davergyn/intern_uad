...
export default function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { adminEmail, isAdmin, logout } = useAuth();
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-900">
        <div className="flex flex-col items-center gap-3">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#CB2229] border-t-transparent" />
          <p className="text-sm font-medium text-slate-400">Memuat panel admin...</p>
        </div>
      </div>
    );
  }
