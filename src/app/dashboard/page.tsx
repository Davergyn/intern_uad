...
export default function UserDashboardPage() {
  ...
  // Load custom profile name and saved events count from localStorage
  useEffect(() => {
    if (isMounted && userEmail) {
      // Name
      const storedProfile = localStorage.getItem(`profile_${userEmail}`);
      if (storedProfile) {
        try {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
        ...
        {/* Profile Card Aside */}
        <aside className="rounded-[2rem] border border-slate-200 bg-slate-900 p-6 text-white shadow-sm sm:p-7 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-5 flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-lg font-bold truncate max-w-[200px]">{profileName}</p>
                <p className="text-sm text-slate-400 truncate max-w-[200px]">{userEmail}</p>
              </div>
            </div>
          ...
          <div className="mt-6 rounded-3xl bg-[#CB2229] p-5 shadow-lg shadow-red-950/20">
            ...
            <p className="mt-2 text-sm leading-6 text-white/80">
              Mulai dari event terdekat, lalu buka materi yang relevan untuk mempercepat progres belajar Anda.
            </p>
          </div>
        </aside>
      </section>
