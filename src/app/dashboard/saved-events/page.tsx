      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-3xl font-black tracking-tight flex items-center gap-2.5">
            <Bookmark className="text-[#CB2229] shrink-0" fill="currentColor" size={28} />
            Event Tersimpan
          </h1>
          <p className="mt-1 text-slate-500">
            Daftar webinar, workshop, atau seminar yang telah Anda simpan/bookmark.
          </p>
        </div>
        <Link
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <span className={`rounded-xl px-2.5 py-1 text-xs font-bold ${TYPE_BADGE[event.type]}`}>
                    {TYPE_LABEL[event.type]}
                  </span>
                  <span className={`inline-flex items-center gap-1 rounded-xl px-2.5 py-1 text-xs font-bold ${DELIVERY_BADGE[event.delivery]}`}>
                ...
                <div className="space-y-2 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600 font-medium">
                  ...
                  {event.start_time && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {event.start_time.substring(0, 5)} - {event.end_time?.substring(0, 5) ?? "Selesai"} WIB
                      </span>
                    </div>
                  )}
                  {event.quota !== null && (
