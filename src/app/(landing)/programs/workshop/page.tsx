...
export const metadata = {
  ...
  description: "Sesi praktik intensif seputar domain .id, keamanan siber, dan transformasi digital.",
};

export default async function WorkshopPage() {
  // 1. Ambil data program workshop
  const programData = await db
    .select()
    .from(programs)
    .where(
      and(eq(programs.kategori, "workshop"), eq(programs.isActive, true))
    );

  // AMBIL SEMUA URL GAMBAR YANG VALID, BUKAN CUMA SATU
